...
**Per-commit verification (mandatory):**
Before staging files, run `git status` and `git diff --stat` to see all modified files — not just the ones you edited in this session segment. Stage files explicitly by name; never use `git add .` or `git add -A`. After the commit, run `git log --oneline -1` to confirm it landed with the correct message before writing the next test.
...
5. Run `git stash list`. If any stash exists, inspect it with `git stash show -p stash@{0}` before applying. Look for project files (`.xcodeproj/`, `.xcscheme`) — these are auto-generated and may represent stale or regressed state. Decide explicitly whether to apply or drop the stash; never blindly pop it.
6. After any rebase or stash pop, run `git diff --stat` against HEAD. Treat any unexpected `.xcodeproj` or `.xcscheme` diff as a warning: if the working-tree version appears older or unrelated, restore it with `git checkout -- <file>` before continuing.
